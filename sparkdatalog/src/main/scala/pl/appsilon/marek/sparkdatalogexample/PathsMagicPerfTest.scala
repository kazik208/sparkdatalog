  ...
  override def runDatalog(): Unit = {
    val query = """
                  ...
                  | Q(x) :- Path(214328887,x).
                		""".stripMargin
    val optimizationResult = MagicSetsOptimizer.optimize(query)
    val facts : Seq[DatalogFact] = optimizationResult.facts
    val fact1 : DatalogFact = facts.head

    val newdb = database.include(Relation.unary(fact1.symbol, sc.parallelize(fact1.variables)))
    val optres = optimizationResult.rules.mkString("\n") + "\n" + optimizationResult.query
    val resultDatabase = newdb.datalog(optres)
